...
void RGBDisplay::GetKeyBoardKeyDimensions(int &x, int &y, int &w, int &h, int &label_x, TouchKbKeys kb_key_flag, int key_array_x = 0, int key_array_y = 0, int cursor_shift_right = 0) {
  switch(kb_key_flag) 
  {
    ...
    case KB_NUMPAD_KEY:
      ...
      break;
    case KB_SAVE_BUTTON:
      x = kSaveButtonX1, y = kSaveButtonY1, w = kSaveButtonW, h = kSaveButtonH, label_x = 7;
      break;
    case KB_BACK_BUTTON:
      x = kBackButtonX1, y = kBackButtonY1, w = kBackButtonW, h = kBackButtonH, label_x = 7;
      break;
  }
}

// credits: Andrew Mascolo https://github.com/AndrewMascolo/Adafruit_Stuff/blob/master/Sketches/Keyboard.ino
void RGBDisplay::MakeKeyboard(std::string label) {
  // clear screen
  tft.fillScreen(kDisplayBackroundColor);
  ...
  if (GetKeyboardPress_numpad) {
    if (GetKeyboardPress_shift) {
      current_keypad_ptr = (char *)Mobile_SymKeys;
    }
    else {
      current_keypad_ptr = (char *)Mobile_NumKeys;
    }
  }
  else {
    if (GetKeyboardPress_shift) {
      current_keypad_ptr = (char *)Mobile_KB_Capitals;
    }
    else {
      current_keypad_ptr = (char *)Mobile_KB_Smalls;
    }
  }
  ...
  for (int y = 0; y < (kb_numbers_only ? 1 : 3); y++) {
    ...
    for (int x = 3; x < kKeypadArrCols; x++) {
      if (x >= x_max)
        break;
      char letter = pgm_read_byte(&(current_keypad_ptr[y * kKeypadArrCols + x]));
      DrawKeyboardButton(KB_ALPHANUMERIC_KEY, /*bool clicked*/ false, x, y, cursor_shift_right, letter);
    }
  }
  ...
  DrawKeyboardButton(KB_SAVE_BUTTON, /*bool clicked*/ false);
...
bool RGBDisplay::GetKeyboardPress(char * textBuffer, std::string label, char * textReturn) {
  ...
  if (ts->IsTouched())
  {
    ...
    for (int y = 0; y < (kb_numbers_only ? 1 : 3); y++)
    {
      ...
      for (int x = 3; x < 13; x++)
      {
        ...
        {// this will draw the button on the screen by so many pixels
          ...
          {
            char pressed_char = pgm_read_byte(&(current_keypad_ptr[y * kKeypadArrCols + x]));
    ...
    if (IsTouchWithin(KB_DELETE_KEY))
    {
      ...
      // clear writing pad
      tft.fillRect(0, 40, tft.width(), kTextAreaHeight - 40, kDisplayBackroundColor);
    ...
    // Enter
    bool enter_key_pressed = IsTouchWithin(KB_ENTER_KEY);
    bool save_key_pressed = IsTouchWithin(KB_SAVE_BUTTON);
    if (enter_key_pressed || save_key_pressed)
    {
      ...
      DrawKeyboardButton((enter_key_pressed ? KB_ENTER_KEY : KB_SAVE_BUTTON), /*bool clicked*/ true);
      delay(kUserInputDelayMs);
      DrawKeyboardButton((enter_key_pressed ? KB_ENTER_KEY : KB_SAVE_BUTTON), /*bool clicked*/ false);
    }
  }
...
void RGBDisplay::TouchCalibrationScreen(int16_t x0, int16_t y0, int16_t x1, int16_t y1, bool touched, bool redraw, bool calibration_done) {
  if(redraw) {
    ...
    std::string title_str, description_str;
    if(!calibration_done) {
      title_str = "Calibrate Touchscreen";
      description_str = "\"Tap\" the shown line using\n a blunt 1-1.5mm dia stylus";
    }
    else {
      title_str = "Re-Calibration done!";
      description_str = "Moving to testing...";
    }
    ...
    tft.setFont(&FreeMono9pt7b);
    tft.setTextColor(kDisplayBackroundColor);
    uint16_t description_str_h = 0, description_str_w = 0;
    int16_t description_str_gap_x = 0, description_str_gap_y = 0;
    tft.getTextBounds(description_str.c_str(), 0, 0, &description_str_gap_x, &description_str_gap_y, &description_str_w, &description_str_h);
    // print text on screen
    tft.setTextColor(kDisplayColorYellow);
    tft.setCursor((kTftWidth - description_str_w) / 2, kTftHeight / 2 + description_str_h / 2);
    tft.print(description_str.c_str());
  }
...
bool RGBDisplay::TouchCalibrationScreenTest(int16_t x_target, int16_t y_target, int16_t x_touch, int16_t y_touch, bool redraw) {
  if(redraw) {
    tft.fillScreen(kDisplayBackroundColor);
    ...
    // print text on screen
    tft.setTextColor(kDisplayColorPurple);
    tft.setCursor((kTftWidth - title_str_w) / 2, kTftHeight / 2 - title_str_h * 1/2);
    tft.print(title_str.c_str());

    std::string description_str = "Tap the shown point once. \n Accurate touches mean good\n Calibration.";
    ...
    // print text on screen
    tft.setTextColor(kDisplayColorYellow);
    tft.setCursor((kTftWidth - description_str_w) / 2, kTftHeight / 2 + title_str_h * 1/2 + 5);
    tft.print(description_str.c_str());

  }

  // draw target and touch
  const int16_t kTargetPoint_r = 2, kTargetCircle_r = 6;
  if(x_touch >= 0 || y_touch >= 0) {
    // touched

    // note if touch is accurate
    bool accurate_touch = false;
    if((abs(x_touch - x_target) <= kTargetPoint_r + kTargetCircle_r) && (abs(y_touch - y_target) <= kTargetPoint_r + kTargetCircle_r))
      accurate_touch = true;

    // draw target point:
    tft.fillCircle(x_target, y_target, kTargetPoint_r, (accurate_touch ? kDisplayColorGreen : kDisplayColorRed));
    // draw target circle:
    tft.drawCircle(x_target, y_target, kTargetCircle_r, (accurate_touch ? kDisplayColorGreen : kDisplayColorRed));
    // draw touch
    tft.fillCircle(x_touch, y_touch, kTargetPoint_r, kDisplayColorCyan);

    return accurate_touch;
  }
  else {
    // only show target
    // draw target point:
    tft.fillCircle(x_target, y_target, kTargetPoint_r, kDisplayColorOrange);
    // draw target circle:
    tft.drawCircle(x_target, y_target, kTargetCircle_r, kDisplayColorOrange);

    return false;
  }
}

void RGBDisplay::DisplayMessage(std::string title_str, std::string message_str) {
  tft.fillScreen(kDisplayBackroundColor);
  ...
  // print text on screen
  tft.setTextColor(kDisplayColorPurple);
  tft.setCursor((kTftWidth - title_str_w) / 2, kTftHeight / 2 - (message_str.length() > 0 ? title_str_h : 0));
  tft.print(title_str.c_str());
  ...
  if(message_str.length() > 0) {
    ...
    tft.setFont(&FreeMono9pt7b);
    tft.setTextColor(kDisplayBackroundColor);
    uint16_t message_str_h = 0, message_str_w = 0;
    int16_t message_str_gap_x = 0, message_str_gap_y = 0;
    tft.getTextBounds(message_str.c_str(), 0, 0, &message_str_gap_x, &message_str_gap_y, &message_str_w, &message_str_h);
    // print text on screen
    tft.setTextColor(kDisplayColorGreen);
    tft.setCursor((kTftWidth - message_str_w) / 2, kTftHeight / 2 + title_str_h * 1/2);
    tft.print(message_str.c_str());
  }
}
